...
int main(int argc, char** argv) {
	...
	if (my_rank==0) {
		ept[0] = a;//atof(argv[1]); //a
		ept[1] = b;//atof(argv[2]); //b
	}
	/*************************/
	MPI_Bcast(ept,2,MPI_FLOAT,0,MPI_COMM_WORLD);
	/*************************/

	//h = (b-a)/n;    /* h is the same for all processes */
	h = (ept[1]-ept[0])/n;    /* h is the same for all processes */
	...
	local_a = ept[0] + my_rank*local_n*h;
	...
	/* Print the result */
	if (my_rank == 0) {
		printf("With n = %d trapezoids, our estimate of \n",n);
