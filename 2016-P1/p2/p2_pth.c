...
// In collision()->set_velocity(), race condition may happen
// when setting position target_particle
pthread_rwlock_t particle_rwlock;


void *thread_move_func(void *args);
void *thread_dist_func(void *args);
void *thread_coll_func(void *args);
...
int main(int argc, char* argv[]) {
	...
	printf("\n=======  Brownian Motion Simulation - Parallel =======\n");
...
void process() {
	...
	// Init rwlock for target_particle in collision()
	pthread_rwlock_init(&particle_rwlock, NULL);

	// Create threads
	pthread_t *thread_list = (pthread_t *) malloc(num_threads * sizeof(pthread_t));

	// move()
	Thread_Move_Param *move_param_list = (Thread_Move_Param *)
	                                     malloc(num_threads * sizeof(Thread_Move_Param));
	int segment_size_per_thread = (num_particles + 1) / num_threads;

	// get_distance()
	Thread_Dist_Param *dist_param_list = (Thread_Dist_Param *)
	                                     malloc(num_threads * sizeof(Thread_Dist_Param));
	int index_step = num_threads;

	for (time_step = 0; time_step < run_time_step; time_step++) {

		// move()
		// Spawn thread and run the move() routine
		for (i = 0; i < num_threads; ++i) {
			// Thread runs [index_start index_end)
			move_param_list[i].index_start = i * segment_size_per_thread;

			// Last thread process the remainder of the particles
			if (i < num_threads - 1)
				move_param_list[i].index_end = (i + 1) * segment_size_per_thread;
			else
				move_param_list[i].index_end = num_particles + 1;

			pthread_create(&thread_list[i], NULL, thread_move_func, &move_param_list[i]);
		}
		// Join the threads
		for (i = 0; i < num_threads; ++i)
		...
		// get_distance()
		// Spawn thread and run the get_distance() routine
		for (i = 0; i < num_threads; ++i) {
			// Thread runs [index_start index_end)
			dist_param_list[i].index_start = i;

			// Last thread process the remainder of the particles
			dist_param_list[i].index_step = index_step;

			pthread_create(&thread_list[i], NULL, thread_dist_func, &dist_param_list[i]);
		}
		// Join the threads
		for (i = 0; i < num_threads; ++i)
			pthread_join(thread_list[i], NULL);
		...
		// 	pthread_create(&thread_list[i], NULL, thread_coll_func, &dist_param_list[i]);
		// }
		// // Join the threads
		// for (i = 0; i < num_threads; ++i)
		// 	pthread_join(thread_list[i], NULL);
	...
	free(distance_table);
	free(move_param_list);
	free(dist_param_list);
}

void *thread_move_func(void *args) {
	Thread_Move_Param *param = (Thread_Move_Param *) args;
...
void *thread_dist_func(void *args) {
	Thread_Dist_Param *param = (Thread_Dist_Param *) args;

	int i, j;
	for (i = param->index_start; i < num_particles + 1; i += param->index_step)
		for (j = i; j < num_particles + 1; j++)
...
void collision(struct Particle *particle, struct Particle *target_particle){
	...
	*/
	...
	//pthread_rwlock_rdlock(&particle_rwlock);
	...
	//pthread_rwlock_unlock(&particle_rwlock);
	...
	//pthread_rwlock_wrlock(&particle_rwlock);
	// Update velocities	
