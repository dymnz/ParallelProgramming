...
inline int is_distance_shorter_after_swap(int start, int end) {
...
void two_opt(int start, int end) {
	...
	int swap_flag = is_distance_shorter_after_swap(start, end);
    ...
    // The *prepare* and *change* cannot be interrupted, race condition may occur			
		pthread_rwlock_wrlock(&route_list_rwlock);
   
    // Before swapping, check again if the condition has changed
    if (!is_distance_shorter_after_swap(start, end))
      return;
