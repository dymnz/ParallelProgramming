/*
	Naive search space division/Proper/No distance cache
*/
#include <stdio.h>
#include <pthread.h>
#include <stdlib.h>
...
void two_opt(int start, int end) {
    ...
    dist_type original_distance = get_route_distance(route_index_list);
	...
	if (get_route_distance(new_route_list) < original_distance) {
...
int main(int argc, char const *argv[])
{
  ...
  for (i = 0; i < num_edge; ++i) {
	...
	printf("Naive search space division/Proper/No Distance cache:\n");
	parallel_2opt();
...
#ifdef ENABLE_2OPT_COUNTER
	printf("2opt_call: %7d 2opt_swap: %7d \%:%.2f\n", 
    opt_counter, 
    swap_counter, 
    (float)swap_counter/opt_counter*100);
#endif
